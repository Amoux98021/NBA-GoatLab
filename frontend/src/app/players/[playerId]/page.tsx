...
import { RankDistributionGraphic } from "@/components/data-visualization";
...
export default async function PlayerPage({ params }: Props) {
  ...
  return <div className="shell interior-page profile-page"><nav className="breadcrumb" aria-label="Breadcrumb"><Link href="/leaderboard">Leaderboard</Link><span aria-hidden="true">/</span><span>{identity.player_name}</span></nav>
    <header className="profile-hero" data-reveal><div><p className="eyebrow">PLAYER PROFILE · {identity.cutoff_season} CUTOFF</p><h1>{identity.player_name}</h1><p className="profile-hero__sub">{career} · {identity.career_state === "ACTIVE" ? "Active · measured to date, no projection" : identity.career_state === "RETIRED" ? "Retired" : "Career status unknown"}</p><div className="profile-hero__actions"><StatusBadge status={identity.ranking_status} /><Link className="button button--light" href={`/compare?playerA=${encodeURIComponent(identity.player_id)}`}>Compare this player <span aria-hidden="true">↗</span></Link></div></div>
    ...
    {leaderboard ? <>
      ...
      <section className="profile-section" data-reveal><div className="section-heading"><div><p className="eyebrow">DISTRIBUTION, NOT A VERDICT</p><h2>Where the evidence places this career</h2></div></div>
        <div className="ranking-grid"><div className="ranking-panel ranking-panel--main"><div className="ranking-panel__eyebrow">RANK DISTRIBUTION <InfoTip label="rank distribution">Rank bands are conditional on the frozen GOATLab methodology. Different historical evidence can yield different precision.</InfoTip></div><div className="large-rank">{rank(leaderboard.rank.median_rank)}<span>typical rank</span></div><RankDistributionGraphic summary={leaderboard.rank} /><div className="band-list"><div><span>50% rank band</span><strong>{rank(leaderboard.rank.lower_50)}–{rank(leaderboard.rank.upper_50)}</strong></div><div><span>80% rank band</span><strong>{rank(leaderboard.rank.lower_80)}–{rank(leaderboard.rank.upper_80)}</strong></div><div><span>90% rank band</span><strong>{rank(leaderboard.rank.lower_90)}–{rank(leaderboard.rank.upper_90)}</strong></div><div><span>95% rank band</span><strong>{rank(leaderboard.rank.lower_95)}–{rank(leaderboard.rank.upper_95)}</strong></div></div></div>
