...
export default async function FullLeaderboard({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="shell interior-page"><header className="interior-header" data-reveal><p className="eyebrow">THE FULL FIELD</p><h1>Explore the leaderboard.</h1><p>Search 1,882 rankable careers drawn from 5,103 player profiles. Filters never alter anyone’s measured value.</p></header>
    <UncertaintyInfo compact />
    <form className="filter-bar" data-reveal action="/leaderboard" method="get" role="search"><label>Search players<input name="search" type="search" defaultValue={search} placeholder="e.g. Kareem Abdul-Jabbar" autoComplete="off" /></label><label>Career<select name="active" defaultValue={active}><option value="">All careers</option><option value="true">Active only</option></select></label><label>Evidence<select name="status" defaultValue={status}><option value="">All statuses</option><option value="OFFICIAL">Official</option><option value="PROVISIONAL">Provisional</option><option value="INTERVAL_NATIVE">Interval-native</option></select></label><button className="button button--dark" type="submit">Apply filters <span aria-hidden="true">↗</span></button></form>
    <div className="results-bar"><strong>{result.total.toLocaleString()} rankable careers</strong><span>{result.total ? `Showing ${result.offset + 1}–${Math.min(result.offset + pageSize, result.total)}` : "No matches"}</span></div>
