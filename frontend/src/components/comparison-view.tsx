...
import { ComparisonDimensionGraphic, ComparisonRankBands } from "./data-visualization";
...
export function ComparisonView({ a, b, comparison }: { a: PlayerProfile; b: PlayerProfile; comparison: PairwiseResponse }) {
  ...
  return <div className="shell interior-page comparison-page">
    ...
    <header className="interior-header" data-reveal><p className="eyebrow">A COMPARISON, WITH UNCERTAINTY</p><h1>{a.identity.player_name} <span className="comparison-vs">vs</span> {b.identity.player_name}</h1><p>A probability under GOATLab’s frozen methodology—not a verdict on who is objectively better.</p></header>
    ...
    <section className="profile-section" data-reveal><div className="section-heading"><div><p className="eyebrow">THE RANK DISTRIBUTIONS</p><h2>Placement is a range.</h2><p>Top-N chances and rank bands come from the frozen release, not from the comparison display.</p></div></div><ComparisonRankBands playerA={a.identity.player_name} playerB={b.identity.player_name} summaryA={comparison.player_a_rank} summaryB={comparison.player_b_rank} /><div className="comparison-rank-grid">{[a, b].map((profile) => { const entry = profile.leaderboard!; return <article key={profile.identity.player_id} className="comparison-rank-card"><h3>{profile.identity.player_name}</h3><dl><div><dt>Median rank</dt><dd>{rank(entry.rank.median_rank)}</dd></div><div><dt>80% rank band</dt><dd>{rank(entry.rank.lower_80)}–{rank(entry.rank.upper_80)}</dd></div><div><dt>90% rank band</dt><dd>{rank(entry.rank.lower_90)}–{rank(entry.rank.upper_90)}</dd></div></dl><div className="comparison-rank-card__probabilities"><Probability label="Top 10" value={entry.top_n.top_10} /><Probability label="Top 25" value={entry.top_n.top_25} /><Probability label="Top 50" value={entry.top_n.top_50} /><Probability label="Top 100" value={entry.top_n.top_100} /></div></article>; })}</div></section>
    <section className="profile-section" data-reveal><div className="section-heading"><div><p className="eyebrow">THE SEVEN FROZEN DIMENSIONS</p><h2>Where the summaries differ.</h2><p>Point values and 90% ranges retain their evidence status. These are not per-dimension head-to-head probabilities.</p></div></div><div className="comparison-dimension-table" role="table" aria-label="Seven-dimension player comparison"><div className="comparison-dimension-table__head" role="row"><span role="columnheader">{a.identity.player_name}</span><span role="columnheader">Dimension</span><span role="columnheader">{b.identity.player_name}</span><span className="sr-only" role="columnheader">Shared zero to 100 visual comparison</span></div>{DIMENSIONS.map((dimension) => { const pair = comparison.dimensions[dimension]; const rangeA = pair.player_a.lower_90 !== null && pair.player_a.upper_90 !== null; const rangeB = pair.player_b.lower_90 !== null && pair.player_b.upper_90 !== null; const overlap = rangeA && rangeB && pair.player_a.lower_90! <= pair.player_b.upper_90! && pair.player_b.lower_90! <= pair.player_a.upper_90!; return <div className="comparison-dimension-row" role="row" key={dimension}><span className="comparison-dimension-row__player comparison-dimension-row__player--a" role="cell" data-player={a.identity.player_name}><DimensionValue profile={a} dimension={dimension} /></span><span className="comparison-dimension-row__name" role="rowheader">{dimension.charAt(0) + dimension.slice(1).toLowerCase()}<small>{rangeA && rangeB ? overlap ? "90% ranges overlap" : "90% ranges do not overlap" : "Evidence precision differs"}</small></span><span className="comparison-dimension-row__player comparison-dimension-row__player--b" role="cell" data-player={b.identity.player_name}><DimensionValue profile={b} dimension={dimension} /></span><span className="comparison-dimension-row__graphic" role="cell"><ComparisonDimensionGraphic playerA={a.identity.player_name} playerB={b.identity.player_name} valueA={pair.player_a} valueB={pair.player_b} /></span></div>; })}</div></section>
    <section className="comparison-interpretation" data-reveal><div><p className="eyebrow">WHAT THE EVIDENCE DOES—AND DOESN’T—SAY</p><h2>Why does GOATLab lean this way?</h2><p>The pairwise probability comes from aligned Overall simulation draws across all seven dimensions. It is not a direct sum of the displayed point differences.</p>{factualPointHigherA.length > 0 && <p>Among dimensions with point summaries for both players, {a.identity.player_name} has a higher displayed point in {factualPointHigherA.join(", ")}.</p>}{factualPointHigherB.length > 0 && <p>{b.identity.player_name} has a higher displayed point in {factualPointHigherB.join(", ")}.</p>}<p>Interval-native dimensions are shown as ranges. GOATLab does not assign a precise numerical contribution or causal “driver” where that would imply unsupported precision.</p></div><div><h3>Measurement context</h3><p>Pairwise probability reflects modeled measurement uncertainty. Historical evidence differs across eras, so close comparisons may remain uncertain even when central summaries differ. Wider uncertainty does not mean a player is worse.</p><p>All probabilities are conditional on the frozen measurement architecture; shared cross-player calibration-model uncertainty is not fully represented.</p></div></section>
