...
| Release files | controlled object storage | deterministic tar.gz, pinned transport SHA-256, manifest/release re-verification |
...
| `GOATLAB_RELEASE_BUNDLE_URL` | HTTPS location of the immutable deployment bundle |
| `GOATLAB_RELEASE_BUNDLE_SHA256` | transport hash printed by `package_ranking_release.py` |
| `GOATLAB_RELEASE_BUNDLE_BEARER_TOKEN` | optional object-store token; secret |
...
Upload the archive without changing bytes, record its printed SHA-256, configure secrets, then run
